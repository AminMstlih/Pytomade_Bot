...
class OKXTrader:
    ...
    def run_strategy(self):
        ...
        positions = self.get_positions()
        if not any(float(p["pos"]) > 0 for p in positions):
            logging.info("No open positions found, checking initial signal...")
            ...
            if signal == "short":
                ...
                logging.info("No clear initial signal")
        ...
        while True:
            try:
                ...
                if signal != last_signal:
                    logging.info(f"New signal detected: {signal.upper()}")
...
if __name__ == "__main__":
    logging.info("Starting OKX Trading Bot with Auto-Start Position")
    try:
        trader = OKXTrader()
        trader.run_strategy()
    except Exception as e:
