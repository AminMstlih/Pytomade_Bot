...
def rf_gb_lstm_strategy():
    global total_profit, successful_trades, failed_trades, entry_price
    ...
    while True:
        ...
        if position:
            ...
            if position == "long":
                profit = (current_price - entry_price) * ORDER_SIZE * LEVERAGE
            else:
                profit = (entry_price - current_price) * ORDER_SIZE * LEVERAGE
            total_profit += profit
            logging.info(f"Current Profit: {profit:.2f} USDT | Total Profit: {total_profit:.2f} USDT")
