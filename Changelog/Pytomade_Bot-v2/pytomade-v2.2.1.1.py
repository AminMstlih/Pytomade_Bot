...
def send_request(method, endpoint, body=None):
    ...
    try:
        ...
        headers = {
            ...
            "x-simulated-trading": "0"  # 1 = Simulated trading mode / 0 = real
        }
...
def ma_cross_ml_strategy():
    ...
    while True:
        ...
        def predicted_price(prices):
            x = np.arange(len(prices)).reshape(-1, 1)
            y = np.array(prices)
            poly = PolynomialFeatures(degree=2)  # Coba degree=2 atau 3
            x_poly = poly.fit_transform(x)
            model = LinearRegression()
            model.fit(x_poly, y)
            return model.predict(poly.transform([[len(prices)]]))[0]
            logging.info(f"Short MA: {short_ma}, Long MA: {long_ma}, Current Price: {current_price}, Predicted Price: {predicted_price}")
            print(f"Short MA: {short_ma}, Long MA: {long_ma}, Current Price: {current_price}, Predicted Price: {predicted_price}")
        predicted = predicted_price(prices)    

        # Logika trading berdasarkan MA Cross dan prediksi ML
        if short_ma > long_ma and predicted > current_price and position != "long":
            # Golden Cross + ARIMA mendukung naik
            close_all_positions()
        ...
        elif short_ma < long_ma and predicted < current_price and position != "short":
            # Death Cross + ARIMA mendukung turun
            close_all_positions()
            ...
            # Tidak ada sinyal yang valid
            logging.info("Sinyal MA tidak didukung oleh ARIMA, menunggu...")

        time.sleep(60)  # Cek setiap 15 detik
