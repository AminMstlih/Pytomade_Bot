...
# Check account balance for hedging
def check_balance():
...
# Place order with USDT-based sizing and multiple TPs
def place_order(side, entry_price):
    if not check_balance():
        logger.error("Insufficient balance for position")
        return None

    # Cap position size
    size = min(POSITION_SIZE_USDT, MAX_POSITION_SIZE)
    size = round(size, 0)  # Round to 2 decimals for USDT
    ...
    # Calculate sizes for TP1 and TP2 in USDT
    tp1_size = round(TP1_SIZE_RATIO * size, 0)
    tp2_size = round((1 - TP1_SIZE_RATIO) * size, 0)
    sl_size = round(size, 0)
    ...
    logger.info(f"Attempting to place {side} order: {size:.2f} USDT, Margin=${margin_used:.2f}, "
                f"Entry=${entry_price:.4f}, TP1=${tp1_price:.4f} ({tp1_size:.2f} USDT), "
                f"TP2=${tp2_price:.4f} ({tp2_size:.2f} USDT), SL=${sl_price:.4f}")
    response = private_request("POST", "/api/v5/trade/order", json.dumps(payload))
    if response["code"] == "0" and response["data"]:
        ord_id = response["data"][0]["ordId"]
        logger.info(f"Order placed successfully: ordId={ord_id}, Size={size:.2f} USDT")
        return {"ordId": ord_id, "size": size, "entry_price": entry_price}
    else:
        logger.error(f"Order placement failed: {response.get('msg', 'Unknown error')} (code: {response['code']})")
        return None

# Close position with cross margin
    ...
    if response["code"] == "0":
        for pos in response["data"]:
            if pos["pos"] != "0":
                ...
                positions[pos_side] = {
                    "side": pos_side,
                    "size": float(pos["pos"]),  # In contracts, for reference
...
def main():
    ...
    if not check_unified_account():
        ...
        return
    if not check_balance():
        logger.error("Insufficient USDT balance for position")
        return
    set_leverage()
            ...
            if signal:
                ...
                if signal not in current_positions:
                    ...
                    if order:
                        ...
                        logger.info(f"Hedging position {signal} opened: {order['size']:.2f} USDT")
                    else:
