...
LEVERAGE = 21
POSITION_SIZE_USDT = 31.5  # Notional size in USDT per position (equivalent to old 1.5 margin * 21 leverage)
MAX_POSITION_SIZE = 10000  # Max position size in USDT
LOT_SIZE = 1.0  # Default, updated dynamically
MIN_SIZE = 1.0  # Default, updated dynamically
CT_VAL = 1.0  # Default contract value (amount of base token per contract), updated dynamically

# Risk Management Parameters
TP1_PNL = 0.05  # +5% PNL for TP1
TP2_PNL = 0.07  # +7% PNL for TP2
TP1_SIZE_RATIO = 0.51  # 51% of position for TP1
SL_PNL = -0.05  # -5% PNL for SL
USE_PNL_BASED = True
...
# Fetch instrument details (lotSz, minSz, ctVal)
def get_instrument_details():
    global LOT_SIZE, MIN_SIZE, CT_VAL
    request_path = f"/api/v5/public/instruments?instType=SWAP&instId={INSTRUMENT}"
    ...
    if response['code'] == '0' and response['data']:
        ...
        logger.info(f"Instrument details for {INSTRUMENT}: lotSz={LOT_SIZE}, minSz={MIN_SIZE}, ctVal={CT_VAL}")
        return True
    logger.warning(f"Failed to fetch instrument details: {response.get('msg', 'Unknown error')}. Using defaults: lotSz=1, minSz=1, ctVal=1")
    return False
...
def check_balance():
    ...
    if response["code"] == "0" and response["data"]:
        avail_bal = float(response["data"][0]["details"][0]["availBal"]) if response["data"][0]["details"] else 0
        margin_required = POSITION_SIZE_USDT / LEVERAGE
        # Check for two positions (hedging)
        total_margin = margin_required * (2 if len(current_positions) < 2 else 1)
        logger.info(f"USDT available balance: ${avail_bal:.2f}, Required margin: ${total_margin:.2f}")
        return avail_bal >= total_margin
...
def set_leverage():
    ...
    if response['code'] == '0':
        logger.info(f"Leverage set to {LEVERAGE}x for {INSTRUMENT} with cross margin")
        return True
...
def calculate_indicators(df):
    ...
    def stochrsi(series, rsi_length=14, stoch_length=14, k=3, d=3):
        ...
        k_values = 100 * (rsi_val - lowest_rsi) / (highest_rsi - lowest_rsi)
...
# Place order with isolated margin for hedging and multiple TPs
def place_order(side, entry_price):
    if not check_balance():
        logger.error("Insufficient balance for hedging")
        return None

    # Calculate contracts based on USDT size
    contracts = POSITION_SIZE_USDT / (entry_price * CT_VAL)
    contracts = min(contracts, MAX_POSITION_SIZE / (entry_price * CT_VAL))
    contracts = max(int(round(contracts / LOT_SIZE)) * LOT_SIZE, MIN_SIZE)

    if contracts < MIN_SIZE:
        logger.error(f"Calculated contracts {contracts:.3f} below minSz {MIN_SIZE}")
        return None

    # Calculate TP1, TP2, and SL prices
    tp1_distance = TP1_PNL / LEVERAGE
    tp2_distance = TP2_PNL / LEVERAGE
    sl_distance = abs(SL_PNL) / LEVERAGE

    if side == "long":
        tp1_price = entry_price * (1 + tp1_distance)
        tp2_price = entry_price * (1 + tp2_distance)
    ...
    # Calculate sizes for TP1 and TP2 in contracts
    tp1_size = int(round(TP1_SIZE_RATIO * contracts / LOT_SIZE)) * LOT_SIZE
    tp2_size = int(round((1 - TP1_SIZE_RATIO) * contracts / LOT_SIZE)) * LOT_SIZE
    sl_size = int(round(contracts / LOT_SIZE)) * LOT_SIZE

    # Adjust TP2 if sum doesn't match total
    if tp1_size + tp2_size != contracts:
        logger.warning(f"TP1+TP2 contracts mismatch: {tp1_size} + {tp2_size} != {contracts}. Adjusting TP2")
        tp2_size = contracts - tp1_size

    tp1_size = max(tp1_size, MIN_SIZE)
    tp2_size = max(tp2_size, MIN_SIZE)
    sl_size = max(sl_size, MIN_SIZE)

    # Define close side for algo orders
    close_side = "sell" if side == "long" else "buy"
    pos_side = side

    # Configure attachAlgoOrds with TP1, TP2, and SL
    attach_algo_ords = [
        {
    ...
    actual_notional = contracts * entry_price * CT_VAL
    margin_used = actual_notional / LEVERAGE
    logger.info(f"Attempting to place {side} order: {contracts:.3f} contracts, Notional=${actual_notional:.2f}, Margin=${margin_used:.2f}, "
                f"Entry=${entry_price:.4f}, TP1=${tp1_price:.4f} ({tp1_size:.3f} contracts), "
                f"TP2=${tp2_price:.4f} ({tp2_size:.3f} contracts), SL=${sl_price:.4f}")
    ...
    if response["code"] == "0" and response["data"]:
        ...
        error_details = response.get('data', [{}])[0].get('sMsg', 'No additional details')
        logger.error(f"Order placement failed: {error_msg} (code: {response['code']}, details: {error_details})")
        return None
...
def main():
    ...
    logger.info("Starting trading bot v3.1.7 for KAITO-USDT-SWAP with USDT-based sizing")
    if not get_instrument_details():
        logger.warning("Using default lotSz, minSz, ctVal; may cause order failures")
    ...
    while True:
        try:
            ...
            if signal:
                # Allow opening positions even if opposite position exists (hedging)
                if signal not in current_positions:
                    price = df["close"].iloc[-1]
                    order = place_order(signal, price)
                    if order:
