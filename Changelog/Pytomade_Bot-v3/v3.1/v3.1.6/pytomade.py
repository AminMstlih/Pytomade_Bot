...
def check_balance():
    ...
    if response["code"] == "0" and response["data"]:
        ...
        return balance >= MARGIN_COST_USD * 1
...
# Calculate TP/SL (kept for reference, though now calculated in place_order)
def calculate_tp_sl(price, volatility, side):
    ...
    if USE_PNL_BASED:
        tp_distance = TP2_PNL / LEVERAGE  # Using TP2_PNL as default if single TP needed
...
def place_order(side, contracts, entry_price):
    ...
    logger.info(f"Attempting to place {side} order with isolated margin: {contracts:.3f} contracts, "
                f"Entry=${entry_price:.4f}, TP1=${tp1_price:.4f} ({tp1_size:.2f}), "
                f"TP2=${tp2_price:.4f} ({tp2_size:.2f}), SL=${sl_price:.4f}")
...
def main():
    ...
    logger.info("Starting trading bot v3.1.6 for DOGE-USDT-SWAP with hedging and multiple TPs")
