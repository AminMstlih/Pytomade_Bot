...
SL_PNL = -0.07  # -7% PNL
USE_PNL_BASED = True
MAX_DAILY_LOSS = -5  # Max daily loss in USD
...
def calculate_indicators(df):
    ...
    def adx(high, low, close, period):
        ...
        plus_di = 100 * (plus_dm.ewm(alpha=1/period).mean() / (tr + 1e-10))
        minus_di = abs(100 * (minus_dm.ewm(alpha=1/period).mean() / (tr + 1e-10)))
    ...
    if df[["ma13", "ma21", "stochrsi_k", "stochrsi_d", "atr", "adx"]].isna().any().any():
        logger.warning("Indicators contain NaN or invalid data")
        return None
...
def get_signal(df):
    ...
    signal = None
    if (ma_long and stochrsi_long and volume_trend and ma_trend_strength > MIN_MA_DIFF and
        latest["stochrsi_k"] < 85 and adx_strong):
        signal = "long"
    elif (ma_short and stochrsi_short and volume_trend and ma_trend_strength > MIN_MA_DIFF and
          latest["stochrsi_k"] > 15 and adx_strong):
        signal = "short"

    strong_volume = latest['volume'] > df['volume'].rolling(window=20).mean().iloc[-1] * 1.5
    ...
    logger.info(f"Volume check: Current={latest['volume']:.2f}, SMA5={df['volume'].rolling(window=5).mean().iloc[-1]:.2f}, "
...
def calculate_contracts(price):
    ...
    contracts = min(max(contracts, 0.01), MAX_CONTRACTS)  # Minimum 0.01 contract
    contracts_rounded = max(round(contracts, 3), 0.01)
...
def place_order(side, contracts, entry_price):
    ...
    if contracts < 0.01:
        logger.error(f"Contract size too small: {contracts:.3f} < 0.01")
        return None

    request_path = f"/api/v5/market/candles?instId={INSTRUMENT}&bar=1m&limit=21"
    response = public_request(request_path)
    ...
    if response['code'] == '0' and response['data']:
        ...
        df_volatility = pd.DataFrame({'close': closes, 'high': highs, 'low': lows, 'volume': volumes})
        df_volatility = calculate_indicators(df_volatility)
        if df_volatility is not None and not df_volatility.empty and not pd.isna(df_volatility['atr'].iloc[-1]):
    ...
    logger.info(f"Attempting to place {side} order: {contracts:.3f} contracts ({contracts * CONTRACT_SIZE:.1f} DOGE), "
    ...
    if response["code"] == "0" and response["data"]:
        ...
        return {"ordId": ord_id, "size": contracts, "entry_price": entry_price, "tp_price": tp_price, "sl_price": sl_price}
    else:
        logger.error(f"Order placement failed: {response.get('msg', 'Unknown error')} (code: {response.get('code', 'N/A')})")
...
def update_trailing_stop(position, current_price):
    if not USE_TRAILING_STOP or not position:
        return position
    side = position["side"]
    if side not in current_positions:
        logger.warning(f"No position found for {side} in current_positions; skipping trailing stop update")
        return position
    ...
    if current_pnl >= TRAILING_STOP_ACTIVATION:
        ...
        if (side == "long" and new_sl_price > position["sl_price"]) or \
           (side == "short" and new_sl_price < position["sl_price"]):
            logger.info(f"Updating trailing stop for {side}: New SL=${new_sl_price:.4f}, Current PNL={current_pnl:.2%}")
                ...
                for order in response["data"]:
                    if order["posSide"] == side:
                        ...
                        if cancel_response["code"] == "0":
                            ...
                            new_response = private_request("POST", "/api/v5/trade/order", json.dumps(new_payload))
                            if new_response["code"] == "0" and new_response["data"]:
                                logger.info(f"New trailing stop OCO placed: SL=${new_sl_price:.4f}")
                                return position
                            logger.error(f"Failed to place new trailing stop OCO: {new_response['msg']}")
    return position
...
def close_position(pos_side):
    response = private_request("GET", f"/api/v5/trade/orders-algo-pending?instId={INSTRUMENT}&ordType=oco")
    if response["code"] == "0" and response["data"]:
        algo_ids = [algo["algoId"] for algo in response["data"] if algo["posSide"] == pos_side]
        if algo_ids:
    ...
    for attempt in range(3):
        ...
        if response["code"] == "0":
            ...
            return True
        logger.error(f"Close position {pos_side} failed (attempt {attempt+1}/3): {response['msg']} (code: {response['code']})")
        time.sleep(10)
    return False

# Check positions
def check_positions(latest_price=None):
        ...
        for pos in response["data"]:
            if pos["pos"] != "0" and pos["avgPx"]:
                try:
                    ...
                    logger.info(f"Current position: {pos_side}, {position['size']:.3f} contracts ({position['size'] * CONTRACT_SIZE:.1f} DOGE), "
                                f"entry ${position['entry_price']:.4f}, posId={pos.get('posId', 'N/A')}, PNL={current_pnl:.2%}")
                except ValueError as e:
                    logger.warning(f"Skipping position with invalid data: {pos} (error: {e})")
        if not positions:
            logger.info("No open positions")
    else:
        logger.error(f"Failed to check positions: {response['msg']} (code: {response['code']})")
    return positions
...
def main():
    ...
    response = private_request("GET", "/api/v5/account/balance?ccy=USDT")
    if response["code"] == "0" and response["data"]:
        daily_start_balance = float(response["data"][0]["details"][0]["cashBal"]) if response["data"][0]["details"] else 0
    ...
    while True:
        try:
            ...
            current_positions = check_positions(latest_price)
            ...
            if response["code"] == "0" and response["data"]:
                ...
                if daily_pnl <= MAX_DAILY_LOSS:
                    logger.warning(f"Daily loss limit reached: ${daily_pnl:.2f} <= ${MAX_DAILY_LOSS:.2f}")
                    for pos_side in list(current_positions.keys()):
                        close_position(pos_side)
                        del current_positions[pos_side]
                    time.sleep(300)
