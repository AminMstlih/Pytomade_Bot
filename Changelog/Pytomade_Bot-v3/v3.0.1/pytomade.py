...
def check_unified_account():
    ...
    if response["code"] == "0":
        ...
        return acct_mode in ["2", "3", "4"]  # 2: Single-currency margin, 3: Multi-currency margin, 4: Portfolio margin
    print(f"Failed to check account mode: {response['msg']} (code: {response['code']})")
    return False
...
def set_leverage():
    ...
    for attempt in range(3):
        ...
        if response['code'] == '0':
            print(f"Leverage set to {LEVERAGE}x for {INSTRUMENT}")
            return True
        print(f"Leverage set error (attempt {attempt+1}/3): {response['msg']} (code: {response['code']})")
        time.sleep(5)
    print("Failed to set leverage after retries. Please set leverage manually via OKX web interface.")
    return False
...
def get_historical_data():
    ...
    if response['code'] == '0':
        ...
        return df
    print(f"Failed to fetch candles: {response['msg']} (code: {response['code']})")
    return None
...
def place_order(side, contracts, entry_price):
    ...
    if response["code"] == "0":
        ...
        if algo_response["code"] == "0":
            ...
            print(f"Placed {side} order: {contracts} contracts at ${entry_price}, TP: ${tp_price}, SL: ${sl_price}")
            return {"ordId": ord_id, "size": contracts, "entry_price": entry_price, "algo_ids": algo_ids}
        print(f"Failed to fetch algo orders: {algo_response['msg']} (code: {algo_response['code']})")
    else:
...
def close_position(pos):
    if pos and "algo_ids" in pos:
        for algo_id in pos["algo_ids"]:
            ...
            if response["code"] != "0":
                print(f"Failed to cancel algo order {algo_id}: {response['msg']} (code: {response['code']})")
    if pos:
        side = "sell" if pos["side"] == "long" else "buy"
        payload = {"instId": INSTRUMENT, "tdMode": "cross", "side": side, "ordType": "market", "sz": str(pos["size"])}
        response = private_request("POST", "/api/v5/trade/order", json.dumps(payload))
        if response["code"] == "0":
            print(f"Closed {pos['side']} position: {pos['size']} contracts at entry ${pos['entry_price']}")
        else:
...
def main():
    ...
    if not check_unified_account():
        print("Account is not in Unified Account mode. Please enable it in OKX settings.")
        return
    # Try setting leverage
    if not set_leverage():
        print("Proceeding without auto-setting leverage. Ensure leverage is set manually to 15x via OKX web interface.")
    while True:
        try:
