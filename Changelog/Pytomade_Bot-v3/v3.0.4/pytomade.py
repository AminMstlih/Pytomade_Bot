...
STOCH_THRESHOLD = 0.01  # 1% threshold for Stoch_K vs Stoch_D
...
def private_request(method, request_path, body=''):
    ...
    for attempt in range(3):
        try:
            ...
            logger.error(f"Private request {method} {request_path} failed (attempt {attempt+1}/3): {e}", extra={'force': True})
            if attempt < 2:
                time.sleep(5)
            else:
...
def public_request(request_path):
    ...
    for attempt in range(3):
        try:
            ...
            logger.error(f"Public request {request_path} failed (attempt {attempt+1}/3): {e}", extra={'force': True})
            if attempt < 2:
                time.sleep(5)
            else:
                return {"code": "1", "msg": f"Network error: {str(e)}"}
...
def calculate_indicators(df):
    ...
    if len(df) >= 2:
        ...
        logger.info(f"MA13-MA21 difference: {ma_diff*100:.2f}%", extra={'force': True})
        if ma_diff < MIN_MA_DIFF:
            logger.warning(f"MA13 and MA21 too close ({ma_diff*100:.2f}% < {MIN_MA_DIFF*100}%); may reduce signal reliability", extra={'force': True})
        stoch_diff = abs(df['stoch_k'].iloc[-1] - df['stoch_d'].iloc[-1]) / max(df['stoch_d'].iloc[-1], 0.01)
        logger.info(f"Stoch_K-Stoch_D difference: {stoch_diff*100:.2f}%", extra={'force': True})
        if stoch_diff < STOCH_THRESHOLD:
            logger.warning(f"Stoch_K and Stoch_D too close ({stoch_diff*100:.2f}% < {STOCH_THRESHOLD*100}%); may prevent signal", extra={'force': True})
    sys.stdout.flush()
    return df
...
def place_order(side, contracts, entry_price):
    if side == "long":
        ...
        tp_price = entry_price * (1 - TP_PNL)
        sl_price = entry_price * (1 - SL_PNL)  # SL higher than entry
    logger.info(f"Calculated TP/SL: TP=${tp_price:.4f}, SL=${sl_price:.4f} for {side} at Entry=${entry_price:.4f}", extra={'force': True})
    payload = {
        "instId": INSTRUMENT,
        "tdMode": "cross",
        ...
        for pos in response["data"]:
            if pos["pos"] != "0" and pos["avgPx"]:
                try:
                    ...
                    logger.warning(f"Skipping position with invalid avgPx: {pos['avgPx']} (posId: {pos['posId']})", extra={'force': True})
